<nav x-data="{ open: true }" class="bg-white shadow">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden md:ml-6 md:flex">
                    ...
                    @hasanyrole('expert')
                    <a href="{{ route('associations.index') }}" class="ml-8 inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium leading-5 focus:outline-none transition duration-150 ease-in-out {{ Route::current()->getName() !== 'associations.index' ? 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 focus:text-gray-700 focus:border-gray-300' : 'border-indigo-500 text-gray-900 focus:border-indigo-700' }}">
                        {{ __('layouts.navigation.associations') }}

                        <span class="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-md text-sm font-medium leading-5 bg-indigo-100 text-indigo-800">{{ \App\Models\Association::where('user_id', auth()->user()->id)->count() }}</span>
    ...
    <div :class="{'block': open, 'hidden': !open}" class="hidden md:hidden">
        <div class="pt-2 pb-3">
            ...
            @hasanyrole('expert')
            <a href="{{ route('associations.index') }}" class="mt-1 block pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300 focus:outline-none focus:text-gray-800 focus:bg-gray-50 focus:border-gray-300 transition duration-150 ease-in-out sm:pl-5 sm:pr-6">{{ __('layouts.navigation.associations') }}</a>
            @endhasanyrole
        </div>
